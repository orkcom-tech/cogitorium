 ...
 *
 * The artwork itself, at 128px, drawn into a 26px box — not a mask and not a
 * reduced redrawing. A simplified small form was tried and it was the wrong
 * call: the tab icon and this are the only two places the mark is ever seen,
 * so simplifying them meant the logo was nowhere the operator actually looks.
 *
 * Over-supplying the resolution is the whole trick. At 26 CSS pixels a retina
 * panel draws 52 or 78 real ones, and this illustration resolves from about 48
 * up — so handing the browser a big raster and letting it reduce beats any
 * bitmap baked at the nominal size. It carries its own ground for the same
 * reason the app icon does: as bare alpha it disappears on a light surface.
 */
export const COG_MARK = '/icon-128.png'
